...
Everything below is exercised by the test suite (`npm test`, 43 tests) and/or the e2e
scenarios (`npm run test:e2e:*`); nothing is claimed without a test.
...
- ADRs 0001–0004 (adapter layer, config file, codex dialect, hermetic testing).
- Design doc: SSE protocol adapter (interfaces, data model, mechanisms).
...
1. **Inbound adaptation is not generalized.** Only the *outbound* event stream is
   ...
   native/foreman HTTP shape per channel.
2. **Protocol adapters cover chat-shaped text only.** `openai-chat`/`openai-responses`
   emit text deltas, tool calls and turn/end events. Richer payloads (images, structured
   outputs, reasoning summaries) pass through as native frames or are dropped per adapter
   rules — they are not translated.
3. **No Anthropic / Google / other dialects.** The registry is general, but only three
   adapters ship.
...
- [ ] **Inbound protocol adaptation** — generalize the adapter contract with a
      `parse(wireChunk) → internal frames` direction, starting with `openai-chat` and
      `openai-responses`, so the gateway can accept protocol-native prompt submissions.
...
- [ ] **Anthropic Messages streaming dialect** — first proof that the registry generalizes
      beyond OpenAI-shaped protocols.

### Later (medium term)

- [ ] **Transparent proxy mode** — full-duplex adaptation (inbound parse + outbound
      format) so foreman can sit invisibly between a protocol-native client and dsh.
